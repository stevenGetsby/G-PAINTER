...
from ..representations import Strivec, MeshExtractResult
...
def to_glb(
    ...
    Args:
        ...
        verbose (bool): Whether to print progress.
    """
    vertices = mesh.vertices.detach().cpu().numpy() if isinstance(mesh.vertices, torch.Tensor) else np.asarray(mesh.vertices)
    faces = mesh.faces.detach().cpu().numpy() if isinstance(mesh.faces, torch.Tensor) else np.asarray(mesh.faces)
