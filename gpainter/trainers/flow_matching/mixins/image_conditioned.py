...
class ImageConditionedMixin:
    ...
    def get_cond(self, cond, cond_is_precomputed=None, **kwargs):
        """
        Get the conditioning data.
        If cond is already precomputed DINOv2 features [B, 1369, 1024], skip encode.
        """
        if cond_is_precomputed is not None and (cond_is_precomputed is True or (isinstance(cond_is_precomputed, (list, torch.Tensor)) and all(cond_is_precomputed))):
            # cond 已经是 [B, 1369, 1024] 特征, 无需编码
            pass
        else:
            cond = self.encode_image(cond)
        kwargs['neg_cond'] = torch.zeros_like(cond)
        kwargs.pop('cond_is_precomputed', None)
        result = super().get_cond(cond, **kwargs)
        return result

    def get_inference_cond(self, cond, cond_is_precomputed=None, **kwargs):
        """
    ...
    def vis_cond(self, cond, cond_is_precomputed=None, **kwargs):
        """
        Visualize the conditioning data.
        """
        if cond_is_precomputed is not None and (cond_is_precomputed is True or (isinstance(cond_is_precomputed, (list, torch.Tensor)) and all(cond_is_precomputed))):
            # 预计算特征 [B, 1369, 1024] 无法可视化, 返回空
            return {}
