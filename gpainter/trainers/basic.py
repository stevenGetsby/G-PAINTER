...
class BasicTrainer(Trainer):
    ...
    def load(self, load_dir, step=0):
        ...
        for name, model in self.models.items():
            ...
            _missing, _unexpected = model.load_state_dict(model_ckpt, strict=False)
            if _missing and self.is_master:
                print(f'  [{name}] missing keys (will use init): {_missing}')
            if _unexpected and self.is_master:
                print(f'  [{name}] unexpected keys (ignored): {_unexpected}')
            if self.fp16_mode == 'inflat_all':
        ...
        try:
            ...
            if self.is_master:
                print(f'\n  [WARN] Optimizer state incompatible ({e}), starting fresh optimizer.')
