    ...
    def run(self):
        ...
        while self.step < self.max_steps:
            ...
            if self.step % self.i_sample == 0:
                ...
                self.snapshot(num_samples=self.num_samples, batch_size=1)
            ...
            if self.is_master:
                ...
                if self.step % self.i_log == 0:
                    print(f'Logging step {self.step}...')
                    ## save to log file
                    log_str = '\n'.join([
                    ...
                    # show with mlflow
                    nan_logs = [l for _, l in log if dict_any(l, lambda x: np.isnan(x))]
                    if len(nan_logs) > 0:
                        print(f"[WARN] step {self.step}: found {len(nan_logs)} log entries containing NaN")
                        # 打印一条样本，帮助定位是哪一项是 NaN
                        try:
                            print("[WARN] example NaN log:", nan_logs[0])
                        except Exception:
                            pass

                    log_show = [l for _, l in log if not dict_any(l, lambda x: np.isnan(x))]
                    if len(log_show) == 0:
                        print(f"[WARN] step {self.step}: all log entries contain NaN, skip tensorboard logging for this interval.")
                        log = []
                    else:
